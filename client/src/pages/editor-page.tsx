        ...
        <div className="grid gap-6 px-5 py-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-8">
          <div className="space-y-4">
            ...
            <p className="max-w-2xl text-sm leading-8 text-white/82">
              Редактор теперь должен ощущаться как игровая workstation: инструменты слева, большой stage в центре,
              настройки справа и preview в отдельном dock.
            </p>
          </div>
          ...
          <Panel className="game-screen bg-transparent">
            <div className="space-y-3">
              ...
              <p className="text-sm leading-7 text-white/78">
                `starsReward` живёт только в admin moderation. Обычный игрок собирает маршрут, сохраняет драфт и, когда всё
                готово, отправляет уровень на review.
              </p>
              {level ? (
                <p className="font-display text-[11px] tracking-[0.22em] text-[#ffd44a]">Current status: {level.status}</p>
              ) : (
                <p className="text-sm text-white/76">Первое сохранение создаст постоянный draft в PostgreSQL.</p>
              )}
            </div>
          </Panel>
        }
