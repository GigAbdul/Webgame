...
export function drawStageObjectSprite({ context, object, neighborObjects, x, y, w, h, fillColor, strokeColor, isActive = false, isUsedOrb = false, alpha = 1, animationTimeMs = 0, editorGuideTop, editorGuideBottom, }) {
    ...
    if (object.type === 'JUMP_ORB' ||
        ...
        drawJumpOrbSprite(context, x, y, w, h, fillColor, strokeColor, isUsedOrb, animationTimeMs, object.type === 'GRAVITY_ORB'
            ? 'greenGravity'
            : object.type === 'BLUE_ORB'
                ? 'blueGravity'
                : object.type === 'DASH_ORB'
...
function drawLetterBlockSprite(context, x, y, w, h, letter) {
...
function normalizeObjectRotationDegrees(degrees) {
    if (!Number.isFinite(degrees)) {
        return 0;
    }
