...
type GameCanvasProps = {
  ...
  onExitToMenu?: (payload: { progressPercent: number; pathPoints?: Array<{ x: number; y: number }> }) => void;
  onStop?: (payload: { progressPercent: number; pathPoints?: Array<{ x: number; y: number }> }) => void;
};
...
export function GameCanvas({
  ...
  const levelBounds = useMemo(() => {
    const maxY = Math.max(
      ...sanitizedLevelObjects.map((object) => object.y + object.h),
      FIXED_LEVEL_START_Y + 4,
    );
    const maxX = Math.max(
      ...sanitizedLevelObjects.map((object) => object.x + object.w),
      autoFinishX,
    );
  ...
  useEffect(() => {
    ...
    const rotateAnimations = new Map<
      string,
      {
        ...
        fallbackPivotX: number;
        fallbackPivotY: number;
        startRotation: number;
        totalDegrees: number;
        startedAt: number;
        durationMs: number;
        easing: MoveTriggerEasing;
        lockObjectRotation: boolean;
      }
    >();
    ...
    let finishPreviewCameraAnchorX: number | null = null;
    let completedCameraAnchorX: number | null = null;
    ...
    let jumpCount = 0;
    let dashOrbState: DashOrbState | null = null;
    ...
    const getPaintGroupRotationCenter = (groupId: number, fallbackX: number, fallbackY: number) => {
    ...
    const startDashOrb = (orb: LevelObject) => {
    ...
    const beginFinishSequence = (elapsedMs: number, sourceCenterY?: number, cameraAnchorXOverride?: number) => {
      ...
      const portalCenterY = clamp(
        sourceCenterY ?? playerCenterY,
        player.h * 0.6,
        PERMANENT_STAGE_FLOOR_Y - player.h * 0.5,
      );
      const sequenceDurationMs = clamp(
        distanceToGateway * FINISH_SEQUENCE_DISTANCE_DURATION_FACTOR_MS,
        FINISH_SEQUENCE_DURATION_MS,
        FINISH_SEQUENCE_MAX_DURATION_MS,
      );
      ...
      const cameraAnchorX =
        typeof cameraAnchorXOverride === 'number'
    ...
    const getFollowCameraX = (deltaSeconds: number) => {
    ...
    const isFinishGatewayVisibleInCamera = (cameraXValue: number) => {
      const finishGatewayWorldX = finishGatewayX * cell;
    ...
    const hasRuntimeObjectsAheadOfPlayer = (playerFrontX: number) =>
      getRuntimeObjectsInRange(playerFrontX + 0.001, finishGatewayX).some((object) => {
    ...
    const loop = (timestamp: number) => {
      ...
      if (currentStatus === 'running' && !isPaused && !finishSequence) {
        ...
        for (const [objectId, animation] of rotateAnimations) {
          ...
          const pivot = getPaintGroupRotationCenter(
            animation.centerGroupId,
            animation.fallbackPivotX,
            animation.fallbackPivotY,
          );
          const rotatedCenter = rotatePointAroundPivot(
        ...
        if (!finishSequence) {
          ...
          if (dashOrbState) {
            ...
            player.rotation = normalizeAngle(
              player.rotation + (horizontalSpeed / radius) * deltaSeconds * Math.sign(player.gravity || 1),
            );
...
function drawPermanentStageFloor(
  context: CanvasRenderingContext2D,
  cameraX: number,
  verticalOffset: number,
  width: number,
...
function drawFinishGateway(
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    context.quadraticCurveTo(
      worldX - cell * (1.4 + index * 0.16),
      startY + cell * (0.42 + sweep),
      worldX,
      endY,
    );
    context.stroke();
...
function rotatePointAroundPivot(pointX: number, pointY: number, pivotX: number, pivotY: number, angleRadians: number) {
...
function getSawHitRadiusFactor(type: LevelData['objects'][number]['type']) {
...
function getBallRampExitCarryVelocity(
  player: PlayerState,
  surface: {
    start: { x: number; y: number };
    end: { x: number; y: number };
  ...
  const desiredVelocity = clamp(
    rampSlope * horizontalSpeed * BALL_RAMP_EXIT_CARRY_FACTOR,
  ...
  const boostedVelocity =
    Math.abs(desiredVelocity) < BALL_RAMP_EXIT_CARRY_MIN
