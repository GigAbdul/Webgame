...
const editorOrbHitboxTypes = new Set(['JUMP_ORB', 'DASH_ORB', 'BLUE_ORB', 'GRAVITY_ORB']);
...
function clampEditorLayer(value) {
...
function getHighestEditorLayer(objects) {
    return objects.reduce((highestLayer, object) => Math.max(highestLayer, clampEditorLayer(object.editorLayer)), MIN_EDITOR_LAYER);
}
function compareEditorLayerDrawOrder(left, right) {
    return clampEditorLayer(right.editorLayer) - clampEditorLayer(left.editorLayer);
}
function getEditorViewportWorldBounds(panX, panY, viewportWidth, viewportHeight, cell, paddingUnits = EDITOR_VIEWPORT_CULL_PADDING_UNITS) {
    if (cell <= 0) {
        return {
...
function objectIntersectsEditorViewport(object, viewport) {
    return (object.x <= viewport.right &&
...
export function LevelEditor({ initialLevel, draftStorageKey, saveLabel = 'Save Draft', onClose, onSave, onSubmit, }) {
    ...
    const [activeEditorLayer, setActiveEditorLayer] = useState(MIN_EDITOR_LAYER);
    ...
    const [saveProgressPercent, setSaveProgressPercent] = useState(null);
    ...
    const updateCursorWorld = useCallback((x, y) => {
    ...
    const drawableLevelObjects = useMemo(() => orderedLevelObjects.map((object) => {
        const previewPosition = dragPreviewState?.positions[object.id];
    ...
    const visibleDrawableObjects = useMemo(() => {
        ...
        const viewportBounds = getEditorViewportWorldBounds(pan.x, pan.y, canvasViewport.width, canvasViewport.height, viewportCell);
    ...
    const nextFreePaintGroupId = useMemo(() => {
        const occupiedIds = new Set(paintGroupIds);
        let groupId = 1;
    ...
    const updateSelectedObjectRotation = (rawValue) => {
        const numericValue = Number(rawValue);
        if (!Number.isFinite(numericValue)) {
            return;
        }
    ...
    const nudgeSelectedObjectRotation = (deltaDegrees) => {
        updateSelectedObject((object) => {
        ...
        if (selectedObjects.length > 1) {
            updateSelectedObjects((objects) => {
                ...
                for (const object of objects) {
                    ...
                    const toggledOrientation = isQuarterAlignedRotation(previousRotation) &&
    ...
    const stepEditorLayer = (direction) => {
        if (direction < 0) {
    ...
    const saveStatusMessage = saveState === 'saving'
        ? saveProgressPercent != null
    ...
    return (_jsxs("div", { className: cn('arcade-editor-workstation editor-workbench flex flex-col space-y-5', isInlineTestMode ? 'editor-workbench--inline-test' : '', isMobileLayout ? 'editor-workbench--mobile' : ''), children: [_jsxs(Panel, { className: "arcade-editor-topbar editor-workbench-toolbar game-screen bg-transparent", children: [_jsxs("div", { className: "editor-workbench-toolbar-main", children: [_jsxs("div", { className: "editor-workbench-heading", children: [_jsx("p", { className: "editor-workbench-kicker font-display text-[11px] tracking-[0.26em] text-[#ffd44a]", children: "Forge Workstation" }), _jsx("h3", { className: "editor-workbench-title mt-2 font-display text-3xl text-white", children: title }), _jsx("p", { className: "editor-workbench-subcopy mt-2 max-w-3xl text-sm leading-7 text-white/72", children: "Compact build surface with on-stage tools, quick paint access, and instant preview." })] }), _jsxs("div", { className: "editor-primary-actions", children: [onClose ? (_jsx("button", { type: "button", className: "editor-close-button", onClick: onClose, "aria-label": "Close editor", title: "Close editor", children: _jsx("span", { "aria-hidden": "true", children: "\u00D7" }) })) : null, _jsx(Button, { onClick: handleSave, disabled: saveState === 'saving', children: saveActionLabel }), onSubmit ? (_jsx(Button, { variant: "secondary", onClick: handleSubmit, disabled: hasStartPositions, title: hasStartPositions ? 'Remove all Start Pos markers before publishing' : undefined, children: "Submit for Review" })) : null, _jsx(Button, { variant: "ghost", onClick: toggleGameplayPreview, children: showPreview ? 'Hide Preview' : 'Preview' })] })] }), _jsxs("div", { className: "editor-workbench-toolbar-meta", children: [_jsx(HintChip, { label: "Tool", value: activeToolLabel }), _jsx(HintChip, { label: "Build Layer", value: activeEditorLayerLabel }), _jsx(HintChip, { label: "Selected", value: selectionLabel }), _jsx(HintChip, { label: "History", value: historyPosition }), _jsx(HintChip, { label: "Theme", value: theme }), _jsx(HintChip, { label: "Objects", value: objectCount })] })] }), _jsxs(Panel, { className: "arcade-editor-stage-panel editor-stage-shell game-screen bg-transparent", children: [_jsxs("div", { ref: stageFrameRef, className: "editor-canvas-frame", style: {
                            ...
                            setMessage('Returned to the editor from preview.');
                                                        ...
                                                        }), _jsx(Button, { variant: "ghost", className: "min-w-[4.25rem]", onClick: assignSelectedObjectToNextFreePaintGroup, children: "Add Group" })] }), _jsxs("div", { className: "editor-inline-actions", children: [_jsx(Button, { variant: "ghost", onClick: () => setIsPaintPopupOpen(true), children: "Open Paint" }), _jsx(Button, { variant: "ghost", onClick: detachSelectedObjectFromPaintGroup, children: "Detach Group" })] })] })) : null, selectedObject.type === 'JUMP_PAD' ? (_jsx("div", { className: "grid gap-3 sm:grid-cols-2", children: _jsxs("div", { children: [_jsx(FieldLabel, { children: "Jump Boost" }), _jsx(Input, { type: "number", step: "0.1", min: "1", value: Number(selectedObject.props.boost ?? 16), onChange: (event) => updateSelectedObject((object) => {
                                                            object.props = {
                                                                ...object.props,
                                                                boost: Number(event.target.value),
                                                            };
                                                        }) })] }) })) : null, selectedObject.type === 'DASH_ORB' ? (_jsx("div", { className: "grid gap-3 sm:grid-cols-2", children: _jsxs("div", { children: [_jsx(FieldLabel, { children: "Dash Speed" }), _jsx(Input, { type: "number", step: "0.5", min: "0.5", value: Number(selectedObject.props.dashSpeed ?? DASH_ORB_SPEED), onChange: (event) => updateSelectedObject((object) => {
                                                            object.props = {
                                                                ...object.props,
                                                            ...
                                                            object.props = {
                                                                ...
                                                                ? `Group ${Number(selectedPaintGroupTriggerObject.props.groupId ?? 1)}`
                                                                : 'Post FX' })] }), _jsxs("div", { className: "grid gap-3 sm:grid-cols-2 lg:grid-cols-3", children: [_jsxs("div", { children: [_jsx(FieldLabel, { children: "Activation" }), _jsx(Select, { value: String(selectedTriggerObject.props.activationMode ?? 'zone'), onChange: (event) => updateSelectedTriggerStringProp('activationMode', event.target.value), children: triggerActivationModeOptions.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) })] }), selectedPaintGroupTriggerObject ? (_jsxs("div", { children: [_jsx(FieldLabel, { children: "Target Group" }), _jsx(Input, { type: "number", min: "1", value: Number(selectedPaintGroupTriggerObject.props.groupId ?? 1), onChange: (event) => updateSelectedTriggerNumericProp('groupId', event.target.value, {
                                                                        min: 1,
                                                                    }) })] })) : null, selectedTriggerObject.type === 'MOVE_TRIGGER' ? (_jsxs(_Fragment, { children: [_jsxs("div", { children: [_jsx(FieldLabel, { children: "Move X" }), _jsx(Input, { type: "number", step: "0.5", value: Number(selectedTriggerObject.props.moveX ?? 2), onChange: (event) => updateSelectedTriggerNumericProp('moveX', event.target.value) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Move Y (up +)" }), _jsx(Input, { type: "number", step: "0.5", value: Number(selectedTriggerObject.props.moveY ?? 0), onChange: (event) => updateSelectedTriggerNumericProp('moveY', event.target.value) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Duration (ms)" }), _jsx(Input, { type: "number", min: "1", value: Number(selectedTriggerObject.props.durationMs ?? 650), onChange: (event) => updateSelectedTriggerNumericProp('durationMs', event.target.value, { min: 1 }) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Easing" }), _jsx(Select, { value: String(selectedTriggerObject.props.easing ?? 'none'), onChange: (event) => updateSelectedTriggerStringProp('easing', event.target.value), children: moveTriggerEasingOptions.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) })] }), _jsxs("div", { className: "sm:col-span-2 lg:col-span-3", children: [_jsx(FieldLabel, { children: "Follow Player" }), _jsxs("div", { className: "editor-trigger-check-grid mt-2", children: [_jsxs("button", { type: "button", className: cn('editor-trigger-check-button', selectedTriggerObject.props.lockToPlayerX ? 'is-active' : ''), onClick: () => updateSelectedTriggerBooleanProp('lockToPlayerX', !Boolean(selectedTriggerObject.props.lockToPlayerX)), children: [_jsx("span", { className: "editor-trigger-check-box" }), _jsx("span", { children: "Player X" })] }), _jsxs("button", { type: "button", className: cn('editor-trigger-check-button', selectedTriggerObject.props.lockToPlayerY ? 'is-active' : ''), onClick: () => updateSelectedTriggerBooleanProp('lockToPlayerY', !Boolean(selectedTriggerObject.props.lockToPlayerY)), children: [_jsx("span", { className: "editor-trigger-check-box" }), _jsx("span", { children: "Player Y" })] })] })] })] })) : null, selectedTriggerObject.type === 'ROTATE_TRIGGER' ? (_jsxs(_Fragment, { children: [_jsxs("div", { children: [_jsx(FieldLabel, { children: "Center Group" }), _jsx(Input, { type: "number", min: "1", value: Number(selectedTriggerObject.props.centerGroupId ?? 1), onChange: (event) => updateSelectedTriggerNumericProp('centerGroupId', event.target.value, {
                                                                                min: 1,
                                                                            }) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Degrees" }), _jsx(Input, { type: "number", step: "15", value: Number(selectedTriggerObject.props.degrees ?? 90), onChange: (event) => updateSelectedTriggerNumericProp('degrees', event.target.value) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Times 360" }), _jsx(Input, { type: "number", min: "0", step: "1", value: Number(selectedTriggerObject.props.times360 ?? 0), onChange: (event) => updateSelectedTriggerNumericProp('times360', event.target.value, { min: 0 }) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Duration (ms)" }), _jsx(Input, { type: "number", min: "1", value: Number(selectedTriggerObject.props.durationMs ?? 650), onChange: (event) => updateSelectedTriggerNumericProp('durationMs', event.target.value, { min: 1 }) })] }), _jsxs("div", { children: [_jsx(FieldLabel, { children: "Easing" }), _jsx(Select, { value: String(selectedTriggerObject.props.easing ?? 'none'), onChange: (event) => updateSelectedTriggerStringProp('easing', event.target.value), children: moveTriggerEasingOptions.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) })] }), _jsxs("div", { className: "sm:col-span-2 lg:col-span-3", children: [_jsx(FieldLabel, { children: "Object Angle" }), _jsx("div", { className: "editor-trigger-check-grid mt-2", children: _jsxs("button", { type: "button", className: cn('editor-trigger-check-button', selectedTriggerObject.props.lockObjectRotation ? 'is-active' : ''), onClick: () => updateSelectedTriggerBooleanProp('lockObjectRotation', !Boolean(selectedTriggerObject.props.lockObjectRotation)), children: [_jsx("span", { className: "editor-trigger-check-box" }), _jsx("span", { children: "Keep Original Angle" })] }) })] })] })) : null, selectedTriggerObject.type === 'ALPHA_TRIGGER' ? (_jsxs("div", { children: [_jsx(FieldLabel, { children: "Alpha" }), _jsx(Input, { type: "number", step: "0.05", min: "0", max: "1", value: Number(selectedTriggerObject.props.alpha ?? 0.35), onChange: (event) => updateSelectedTriggerNumericProp('alpha', event.target.value, { min: 0, max: 1 }) })] })) : null, selectedTriggerObject.type === 'TOGGLE_TRIGGER' ? (_jsxs("div", { className: "sm:col-span-2", children: [_jsx(FieldLabel, { children: "Toggle Result" }), _jsxs("div", { className: "editor-inline-actions", children: [_jsx(Button, { variant: selectedTriggerObject.props.enabled ? 'primary' : 'ghost', onClick: () => updateSelectedObject((object) => {
...
function drawEditorObjectHitbox(context, object, playerMode, panX, panY, cell, viewportHeight, alpha = 1) {
    ...
    if (isCollidableBlockType(object.type)) {
        for (const band of getEditorBlockSupportRects(object)) {
            ...
            drawEditorHitboxRect(context, bandPosition.x, bandPosition.y, band.w * cell, band.h * cell, 'rgba(92, 176, 255, 0.14)', 'rgba(92, 176, 255, 0.96)', alpha);
        }
        return;
    }
    const position = worldToScreen(object.x, object.y, panX, panY, cell);
    ...
    drawEditorHitboxRect(context, position.x, position.y, object.w * cell, object.h * cell, fillColor, strokeColor, alpha, strokeDash);
}
function getEditorBlockSupportRects(object) {
    const collisionMask = getBlockCollisionMask(object.type);
    if (!collisionMask || !hasBlockSupport(collisionMask)) {
        return [];
    }
    const thickness = Math.min(Math.max(EDITOR_BLOCK_SUPPORT_BAND_THICKNESS_UNITS, Math.min(object.w, object.h) * 0.18), Math.min(object.w, object.h));
    const bands = [];
...
function normalizeObjectRotationDegrees(value) {
...
function isQuarterAlignedRotation(value) {
