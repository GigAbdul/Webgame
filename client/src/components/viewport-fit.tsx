...
export function ViewportFit({
  ...
  return (
    <div ref={frameRef} className={cn('viewport-fit-frame', className)}>
      <div
        ref={contentRef}
        className={cn('viewport-fit-content', contentClassName)}
        style={{ '--viewport-fit-scale': String(scale) } as CSSProperties}
      >
        {children}
      </div>
    </div>
