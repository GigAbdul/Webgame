...
const difficultyPresentationMap: Record<Difficulty, DifficultyPresentation> = {
  EASY: {
    label: 'Easy',
    shortLabel: 'E',
    meter: 24,
...
const unratedPresentation: DifficultyPresentation = {
  label: 'Unrated',
  shortLabel: '?',
  meter: 18,
  rewardStars: 0,
...
export function getDifficultyPresentation(difficulty: Level['difficulty']) {
  if (!difficulty) {
...
export function getDifficultyStars(difficulty: Level['difficulty']) {
  return getDifficultyPresentation(difficulty).rewardStars;
}

export function getDisplayedStars(level: Pick<Level, 'difficulty' | 'starsReward'>) {
  const autoStars = getDifficultyStars(level.difficulty);
  return autoStars > 0 ? autoStars : level.starsReward;
}

export function getDifficultySpritePath(difficulty: Level['difficulty']) {
  return `/difficulty-icons/${getDifficultyPresentation(difficulty).spriteSlug}.png`;
}

export function formatStageNumber(index: number) {
